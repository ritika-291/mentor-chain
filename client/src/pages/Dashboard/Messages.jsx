      ...
      <div className="flex h-[75vh] bg-gray-800 rounded-xl shadow-2xl border border-gray-700 overflow-hidden">
        ...
        <div className="w-full sm:w-1/3 border-r border-gray-700 overflow-y-auto bg-gray-800/50">
          <div className="p-4 border-b border-gray-700">
            <h3 className="text-xl font-semibold text-white">Active Connections</h3>
          </div>
            ...
            {conversations.length === 0 ? (
              ...
              conversations.map((contact) => (
                <li
                  ...
                  className={`p-4 cursor-pointer transition-colors duration-150 ${activeChatUser?.id === contact.id ? 'bg-teal-900/30 border-l-4 border-teal-500' : 'hover:bg-gray-700'
                    }`}
                >
                  <div className="flex justify-between items-center">
                    <span className={`text-lg font-bold ${activeChatUser?.id === contact.id ? 'text-teal-400' : 'text-gray-200'}`}>
                      {contact.name}
                    </span>
                  </div>
                </li>
